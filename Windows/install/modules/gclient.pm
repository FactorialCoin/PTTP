...
sub openconnection {
  ...
  if ((!defined $iaddr) || (length($iaddr)!=4)) {
    ...
    setsockopt($sock,SOL_SOCKET, SO_SNDTIMEO, 3);
  } else {
    $self->{socket}=undef;
...
sub http {
  ...
  my ($host,$port,$loopmode,$caller,$timeout,$ssl,$sni) = @_;
  my $path=$_[7]; my $query=$_[8]; my $user=$_[9]; my $pass=$_[10];
...
sub website {
  ...
  my $self=http($info->{host},$info->{port},1,\&handle_website,undef,$info->{ssl},undef,$info->{path},$info->{query},$user,$pass);
  return $self 
}
...
sub websocket {
  my ($host,$port,$loopmode,$caller,$ssl,$timeout,$sni) = @_;
